...
func Listen(wg *sync.WaitGroup, info *Info) {
...
func SendRequest(wg *sync.WaitGroup, info *Info, peer *Peer, dhInfo *DH) {
...
func Write() {
	for {
		if info.connection != nil {
			...
			key, salt := aes.deriveKey(dhInfo.SharedSecret, nil)
			textTosend, _ := encrypt(key, text)
			fmt.Println("enc tetToSend:", textTosend)
...
func Read() {
	for {
		if info.connection != nil {
			...
			key, _ := aes128.deriveKey(dhInfo.SharedSecret, mes.Salt)
